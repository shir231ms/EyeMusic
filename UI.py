...
###
	...
	# we define the gaze() in "Eyetribe" class, return "fix"=> true or false
      ...
      #testing
        ...
        #////////////////////////// # 
        if gazePt[0]>=p_pre[0] and gazePt[1]>=Next_Page_Button[2]-2*border and gazePt[1]<=Next_Page_Button[3]+border:
            n_continuous = n_continuous + 1
            print "last line  n_continuous: " , n_continuous
            if n_continuous == 20: #continuous for # points
                if gazePt[0]>=Next_Page_Button[0]-border and gazePt[0]<=Next_Page_Button[1]+border and gazePt[1]>=Next_Page_Button[2]-border and gazePt[1]<=Next_Page_Button[3]+border and gaze and Nextbool:
                    cntNex=cntNex+1
                    cntPre=0
                    cntHig=0
                ...
                # action_2: highlight the meature which my gaze is following
                else:
                    cntHig=cntHig+1
                    if cntHig>=20:
                        cntPre=0
...
def drawRecords(string, sleeptime, *args):
    ...
    while(True):
        ...
        if gazePt[0]==0.0 and gazePt[1]==0.0: 
            continue
        gX.append(gazePt[0])
        gY.append(gazePt[1])
        rainbow = cmap(int(np.rint((float(point_cnt)/PointNumPerImage)*255))) # float/int-> 0.xxxxxxxxxxxx(.12 digits)
...
if __name__ == '__main__':
    global fig, ax, screen_x, screen_y
    fig = plt.figure()
